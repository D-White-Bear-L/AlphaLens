# AlphaLens - 新闻关系溯源和金融量化分析 Agent 平台

<div align="center">

<img src="doc/logo.jpg" alt="AlphaLens Logo" width="200">

[![Python](https://img.shields.io/badge/Python-3.8+-blue.svg)](https://www.python.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-0.104+-green.svg)](https://fastapi.tiangolo.com/)
[![Vue](https://img.shields.io/badge/Vue-3.5-4FC08D.svg)](https://vuejs.org/)
[![License](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

一个基于大语言模型的智能 Agent 平台，集成新闻溯源和金融量化分析两大核心模块。


</div>

---

## 📋 目录

- [项目简介](#项目简介)
- [核心特性](#核心特性)
- [系统架构](#系统架构)
- [技术栈](#技术栈)
- [快速开始](#快速开始)
- [项目结构](#项目结构)
- [API 文档](#api-文档)
- [使用示例](#使用示例)
- [开发指南](#开发指南)
- [贡献指南](#贡献指南)
- [许可证](#许可证)

---

## 项目简介

AlphaLens 是一个基于 LLM（大语言模型）的智能 Agent 平台，提供两大核心功能模块：

### 🔍 新闻溯源模块 (News Trace Agent)

通过多轮迭代的深度搜索和内容提取，自动追踪新闻来源、构建时间线、分析因果关系，并生成知识图谱。支持 1-4 级可配置溯源深度，使用 Playwright 处理动态网页，提取完整内容。

### 💰 金融量化模块 (Financial Agent)

结合传统量化分析技术与现代 AI 能力，提供股票分析、交易信号生成、策略回测和机器学习价格预测功能。支持多种技术指标（MA、RSI、MACD、Bollinger Bands）和交易策略。

---

## 核心特性

### 新闻溯源模块

- 🔍 **深度溯源**: 通过多轮迭代搜索和内容提取，深入追踪新闻来源
- 📊 **结构化分析**: 自动提取时间线、因果关系、知识图谱等多维度信息
- 🕷️ **智能爬虫**: 使用 Playwright 处理动态网页，提取完整内容
- 🎯 **可配置深度**: 支持 1-4 级溯源深度控制，平衡速度与准确性
- 📈 **可视化展示**: 前端提供知识图谱、层级图、时间线等丰富的可视化
- ⚡ **异步任务**: 支持长时间运行的异步任务，实时进度更新，可随时取消

### 金融量化模块

- 🤖 **AI 驱动的分析**: 使用 LLM 生成专业的投资建议和风险评估
- 📊 **多维度技术分析**: 支持 MA、RSI、MACD、Bollinger Bands 等多种技术指标
- 🎯 **动态信号强度**: 基于指标偏离度、趋势一致性等动态计算信号强度
- 🔄 **策略回测引擎**: 支持多种交易策略的历史回测（signal_based、ma_cross、rsi、macd）
- 📈 **股票推荐系统**: 基于多因子评分模型的智能股票推荐
- 🧠 **机器学习预测**: 使用多种 ML 模型预测股票未来价格走势，提供置信区间

---

## 系统架构

```
┌─────────────────────────────────────────────────────────┐
│                     前端 (Vue 3)                          │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │  ChatView    │  │ NewsTrace    │  │ Financial    │  │
│  │  (对话界面)   │  │  Result      │  │  Result      │  │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  │
└─────────┼─────────────────┼─────────────────┼──────────┘
           │                 │                 │
           └─────────────────┼─────────────────┘
                             │ HTTP/REST API
┌────────────────────────────┼─────────────────────────────┐
│                   后端 (FastAPI)                          │
│  ┌──────────────────────────────────────────────────┐   │
│  │              API Routes                          │   │
│  │  - /trace/* (新闻溯源接口)                        │   │
│  │  - /api/v1/financial/* (金融分析接口)            │   │
│  └──────────────────┬───────────────────────────────┘   │
│                     │                                    │
│  ┌──────────────────▼───────────────────────────────┐   │
│  │          Agent Layer                             │   │
│  │  ┌──────────────┐      ┌──────────────┐          │   │
│  │  │NewsTraceAgent│      │FinancialAgent│          │   │
│  │  └──────┬───────┘      └──────┬───────┘          │   │
│  │         │                      │                   │   │
│  │  ┌──────▼───────┐      ┌──────▼───────┐          │   │
│  │  │   Tools       │      │   Modules     │          │   │
│  │  │-GoogleSearch  │      │-StrategyBacktest│        │   │
│  │  │-WebScraper   │      │-StockPrediction│         │   │
│  │  └──────────────┘      └───────────────┘          │   │
│  └──────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```

---

## 技术栈

### 后端技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| **FastAPI** | >=0.104.0 | 异步 Web 框架，提供 RESTful API |
| **mira** | latest | LLM 工具调用框架，支持结构化输出 |
| **Playwright** | >=1.40.0 | 无头浏览器，处理动态网页爬取 |
| **Pydantic** | >=2.0.0 | 数据验证和结构化输出模型 |
| **pandas** | - | 数据处理和分析 |
| **numpy** | - | 数值计算 |
| **akshare** | - | 股票数据获取 |
| **scikit-learn** | - | 机器学习模型 |
| **httpx** | >=0.24.0 | 异步 HTTP 客户端 |
| **loguru** | >=0.7.0 | 结构化日志记录 |

### 前端技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| **Vue 3** | ^3.5.24 | 渐进式前端框架 |
| **Element Plus** | ^2.12.0 | Vue 3 UI 组件库 |
| **Vite** | ^7.2.4 | 快速构建工具 |
| **vis.js** | - | 知识图谱和层级图可视化 |
| **axios** | ^1.13.2 | HTTP 客户端 |
| **vue-router** | ^4.6.4 | 路由管理 |

---

## 快速开始

### 环境要求

- Python 3.8+
- Node.js 16+
- Playwright (安装后需要运行 `playwright install`)

### 1. 克隆项目

```bash
git clone https://github.com/your-username/ai-agent-platform.git
cd ai-agent-platform
```

### 2. 后端设置

#### 安装依赖

```bash
cd backend
pip install -r requirements.txt

# 安装 Playwright 浏览器
playwright install chromium
```

#### 配置环境变量

创建 `.env` 文件：

```env
# LLM 配置
API_KEY=your_api_key
BASE_URL=https://ark.cn-beijing.volces.com/api/v3
MODEL=doubao/ep-20250615233434-wps9w

# Serper API (Google 搜索，新闻溯源模块需要)
SERPER_API_KEY=your_serper_api_key

# 数据库 (可选)
DATABASE_URL=sqlite:///./news_trace.db
```

#### 启动服务

```bash
python run.py
# 或
uvicorn app.main:app --reload --port 8000
```

后端服务将在 `http://localhost:8000` 启动。

### 3. 前端设置

#### 安装依赖

```bash
cd frontend/new_trace_frontend
npm install
```

#### 启动开发服务器

```bash
npm run dev
```

前端应用将在 `http://localhost:5173` 启动。

### 4. 访问应用

打开浏览器访问 `http://localhost:5173`，即可开始使用。

---

## 项目结构

```
AlphaLens/
├── backend/                          # 后端服务
│   ├── app/
│   │   ├── agent/                    # Agent 模块
│   │   │   ├── news_trace_agent.py  # 新闻溯源 Agent
│   │   │   ├── financial_agent.py   # 金融分析 Agent
│   │   │   ├── strategy_backtest.py # 策略回测引擎
│   │   │   └── stock_prediction.py  # 股票预测引擎
│   │   ├── api/                      # API 路由
│   │   │   └── routes.py
│   │   ├── tools/                    # 工具模块
│   │   │   ├── google_search.py     # Google 搜索工具
│   │   │   ├── web_scraper.py       # 网页爬虫工具
│   │   │   └── database_search.py   # 数据库搜索工具
│   │   ├── models.py                # 数据模型
│   │   ├── config.py                # 配置管理
│   │   ├── main.py                  # FastAPI 应用入口
│   │   └── database.py             # 数据库连接
│   ├── requirements.txt
│   ├── run.py                       # 启动脚本
│   └── README.md
│
├── frontend/                         # 前端应用
│   └── new_trace_frontend/
│       ├── src/
│       │   ├── views/               # 页面视图
│       │   │   ├── Chat/           # 聊天界面
│       │   │   └── newTrace/       # 新闻溯源视图
│       │   ├── components/         # 组件
│       │   │   ├── NewsTraceResult.vue    # 溯源结果展示
│       │   │   ├── FinancialResult.vue     # 金融分析结果
│       │   │   ├── BacktestResult.vue     # 回测结果
│       │   │   └── PredictionResult.vue   # 预测结果
│       │   ├── api/                # API 调用
│       │   │   └── agent.js
│       │   └── utils/              # 工具函数
│       ├── package.json
│       └── vite.config.js
│
├── NEW_TRACE_AGENT_README.md        # 新闻溯源模块详细文档
├── FINANCIAL_AGENT_README.md        # 金融量化模块详细文档
└── README.md                        # 本文件
```

---

## API 文档

### 新闻溯源 API

#### 创建溯源任务

```http
POST /trace/async
Content-Type: application/json

{
  "claim": "某科技公司宣布获得新一轮融资",
  "max_depth": 2
}
```

**响应**:

```json
{
  "task_id": "uuid-string",
  "status": "pending"
}
```

#### 查询任务状态

```http
GET /trace/status/{task_id}
```

#### 取消任务

```http
POST /trace/cancel/{task_id}
```

### 金融分析 API

#### 股票分析

```http
POST /api/v1/financial/analyze
Content-Type: application/json

{
  "stock_code": "000001",
  "start_date": "20240101",
  "end_date": "20241226"
}
```

#### 策略回测

```http
POST /api/v1/financial/backtest
Content-Type: application/json

{
  "stock_code": "000001",
  "start_date": "20240101",
  "end_date": "20241226",
  "strategy_type": "signal_based",
  "initial_capital": 100000.0,
  "shares_per_trade": 100,
  "min_signal_strength": 0.5
}
```

#### 股票价格预测

```http
POST /api/v1/financial/predict
Content-Type: application/json

{
  "stock_code": "000001",
  "start_date": "20240101",
  "end_date": "20241226",
  "prediction_days": 5,
  "model_type": "ensemble",
  "use_technical_indicators": true
}
```

#### 股票推荐

```http
POST /api/v1/financial/recommend
Content-Type: application/json

{
  "max_stocks": 10,
  "start_date": "20240101",
  "end_date": "20241226"
}
```

### API 文档访问

启动后端服务后，访问以下地址查看完整的 API 文档：

- Swagger UI: `http://localhost:8000/docs`
- ReDoc: `http://localhost:8000/redoc`

---

## 使用示例

### 新闻溯源示例

```python
from app.agent.news_trace_agent import NewsTraceAgent

agent = NewsTraceAgent()

# 执行溯源
result = await agent.trace_news(
    claim="某科技公司宣布获得新一轮融资",
    max_iterations=6
)

# 查看来源
for source in result.sources:
    print(f"{source.title}: {source.url}")

# 查看时间线
for event in result.timeline:
    print(f"{event.date}: {event.event}")
```

### 金融分析示例

```python
from app.agent.financial_agent import FinancialAgent

agent = FinancialAgent()

# 分析股票
result = await agent.analyze_stock(
    stock_code="000001",
    start_date="20240101",
    end_date="20241226"
)

# 查看交易信号
for signal in result.trading_signals:
    print(f"{signal.signal_type}: {signal.signal_reason} (强度: {signal.signal_strength:.2f})")

# 查看 LLM 生成的评估
print(result.overall_assessment)
```

### 策略回测示例

```python
from app.agent.strategy_backtest import StrategyBacktest
from app.models import BacktestRequest

backtest = StrategyBacktest()

request = BacktestRequest(
    stock_code="000001",
    start_date="20240101",
    end_date="20241226",
    strategy_type="ma_cross",
    initial_capital=100000.0,
    shares_per_trade=100,
    min_signal_strength=0.5
)

result = await backtest.backtest_ma_cross_strategy(request)

print(f"总收益率: {result.metrics.total_return_rate:.2f}%")
print(f"胜率: {result.metrics.win_rate:.2%}")
print(f"最大回撤: {result.metrics.max_drawdown:.2f}%")
```

### 股票预测示例

```python
from app.agent.stock_prediction import StockPrediction
from app.models import PredictionRequest

prediction = StockPrediction()

request = PredictionRequest(
    stock_code="000001",
    start_date="20240101",
    end_date="20241226",
    prediction_days=5,
    model_type="ensemble",
    use_technical_indicators=True
)

result = await prediction.predict_stock_price(request)

for pred in result.predictions:
    print(f"{pred.date}: 预测价格 {pred.predicted_price:.2f} "
          f"(置信区间: {pred.confidence_interval_lower:.2f} - {pred.confidence_interval_upper:.2f})")
```

---

## 开发指南

### 后端开发

1. **代码风格**: 遵循 PEP 8 Python 代码规范
2. **类型提示**: 使用类型提示提高代码可读性
3. **异步编程**: 优先使用 `async/await` 处理 IO 操作
4. **错误处理**: 使用 `try-except` 和 `loguru` 记录错误日志

### 前端开发

1. **组件化**: 使用 Vue 3 Composition API (`<script setup>`)
2. **响应式设计**: 使用 `ref` 和 `computed` 管理状态
3. **代码规范**: 遵循 Vue 3 官方风格指南

### 测试

```bash
# 后端测试
cd backend
pytest

# 前端测试
cd frontend/new_trace_frontend
npm run test
```

---

## 贡献指南

我们欢迎所有形式的贡献！请遵循以下步骤：

1. **Fork 本仓库**
2. **创建特性分支** (`git checkout -b feature/AmazingFeature`)
3. **提交更改** (`git commit -m 'Add some AmazingFeature'`)
4. **推送到分支** (`git push origin feature/AmazingFeature`)
5. **开启 Pull Request**

### 贡献类型

- 🐛 Bug 修复
- ✨ 新功能
- 📝 文档改进
- 🎨 UI/UX 改进
- ⚡ 性能优化
- 🔧 代码重构

### 代码提交规范

请遵循 [Conventional Commits](https://www.conventionalcommits.org/) 规范：

- `feat`: 新功能
- `fix`: Bug 修复
- `docs`: 文档更新
- `style`: 代码格式调整
- `refactor`: 代码重构
- `perf`: 性能优化
- `test`: 测试相关
- `chore`: 构建/工具相关

---

## 技术亮点

### 新闻溯源模块

- **多轮迭代**: 通过 `max_iterations` 控制迭代次数，每次迭代 LLM 可以调用多个工具
- **动态网页处理**: Playwright 可以处理 JavaScript 渲染的页面
- **智能内容提取**: 自动识别文章主内容，过滤无关信息
- **结构化输出**: 使用 Pydantic 和 LLMJson 实现强类型结构化输出

### 金融量化模块

- **动态信号强度**: 基于指标偏离度动态计算信号强度，而非硬编码
- **多策略回测**: 支持 signal_based、ma_cross、rsi、macd 四种策略
- **机器学习预测**: 多维度特征工程，支持多种 ML 模型
- **AI 驱动分析**: LLM 生成专业的投资建议和风险评估

---

## 未来规划

### 新闻溯源模块

- [ ] 支持更多数据源（Twitter、Reddit 等）
- [ ] 实现更智能的内容去重
- [ ] 添加来源权威性评分
- [ ] 支持批量溯源任务
- [ ] 添加导出功能（PDF、JSON）

### 金融量化模块

- [ ] 深度学习模型（LSTM/GRU、Transformer）
- [ ] 更多交易策略（网格交易、动量策略等）
- [ ] 实时数据流处理
- [ ] 多股票组合分析
- [ ] 超参数自动调优

---

## 常见问题 (FAQ)

### Q: 如何获取 Serper API Key？

A: 访问 [Serper.dev](https://serper.dev/) 注册账号并获取 API Key。

### Q: 支持哪些 LLM 提供商？

A: 通过 `mira` 库支持所有 OpenRouter 兼容的 LLM 提供商，包括 OpenAI、Anthropic 等。

### Q: 如何修改溯源深度？

A: 在创建溯源任务时，设置 `max_depth` 参数（1-4），对应不同的迭代次数。

### Q: 金融分析支持哪些股票市场？

A: 目前支持 A 股市场，通过 `akshare` 获取数据。股票代码格式为 6 位数字（如 "000001"）。

---

## 许可证

本项目采用 [MIT License](LICENSE) 许可证。

---

## 致谢

- [FastAPI](https://fastapi.tiangolo.com/) - 现代化的 Python Web 框架
- [Vue.js](https://vuejs.org/) - 渐进式 JavaScript 框架
- [mira](https://github.com/mira-project/mira) - LLM 工具调用框架
- [Playwright](https://playwright.dev/) - 浏览器自动化工具
- [akshare](https://akshare.readthedocs.io/) - 金融数据接口库

---

## 联系方式

- **Issues**: [GitHub Issues](https://github.com/D-White-Bear-L/AlphaLens/issues)
- **Discussions**: [GitHub Discussions](https://github.com/D-White-Bear-L/AlphaLens/discussions)

---

<div align="center">

**如果这个项目对你有帮助，请给一个 ⭐ Star！**

Made with ❤️ by the [AlphaLens](https://github.com/D-White-Bear-L/AlphaLens) team - [D-White-Bear-L](https://github.com/D-White-Bear-L).

</div>

